...
function createSender(ws, wantsZlib) {
    return (d) => {
        if (ws.readyState !== 1) return
        const json = JSON.stringify(d)
        if (!wantsZlib) return ws.send(json)

        let buffer = zlib.deflateSync(json, {
            chunkSize: 65535,
        ...
        if (!ws.zlibHeaderSent) ws.zlibHeaderSent = true
        else buffer = buffer.subarray(2)
...
const send = (ws, d) => {
    if (ws.readyState !== 1) return
    if (ws.sendPayload) ws.sendPayload(d)
    else ws.send(JSON.stringify(d))
}
...
const friends = s => getUsers(Array.isArray(s.friends) ? s.friends : [])
const pendingIncoming = s => getUsers(Array.isArray(s.pending_incoming) ? s.pending_incoming : [])
const pendingOutgoing = s => getUsers(Array.isArray(s.pending_outgoing) ? s.pending_outgoing : [])
const blocked = s => getUsers(Array.isArray(s.blocked) ? s.blocked : [])
...
async function getDMChannel(userA, userB) {
    ...
    const [rows] = await db.execute(
        `SELECT id FROM channels WHERE user_id_1=? AND user_id_2=? LIMIT 1`,
        [user_id_1, user_id_2]
    )
    ...
    try {
        await db.execute(
            `INSERT INTO channels (id, type, user_id_1, user_id_2, created_at) VALUES (?, 1, ?, ?, NOW())`,
...
async function getGroupChannels(userId) {
    const [channels] = await db.execute(
        `SELECT id, type, owner_id, name, icon FROM group_channels
         WHERE id IN (SELECT channel_id FROM group_channel_recipients WHERE user_id=?)
         ORDER BY created_at`,
        [userId]
    )
    ...
    for (const channel of channels) {
        const [rows] = await db.execute(
            `SELECT u.id, u.username, u.discriminator, u.email, u.settings
             FROM group_channel_recipients r JOIN users u ON u.id=r.user_id
             WHERE r.channel_id=? AND r.user_id!=? ORDER BY r.user_id`,
            [channel.id, userId]
        )
        ...
        try {
            ...
            if (!ids.includes(userId)) {
                const [channel] = await db.execute(
                    `SELECT user_id_1, user_id_2 FROM channels WHERE id=? LIMIT 1`,
                    [channelId]
                )
                if (channel.length) ids.push(String(channel[0].user_id_1), String(channel[0].user_id_2))
            }

            for (const client of wss.clients) {
                if (client.readyState === 1 && client.user && ids.includes(String(client.user.id)) && String(client.user.id) !== userId) {
                    send(client, {
                        op: 0, s: ++client.seq, t: 'TYPING_START',
                        d: { channel_id: channelId, user_id: userId, timestamp: Math.floor(Date.now() / 1000) }
                    })
                }
            }
    ...
    try {
        ...
        for (const client of wss.clients) {
            if (client.readyState === 1 && client.user)
                send(client, { op: 0, s: ++client.seq, t: 'MESSAGE_CREATE', d: message })
        }
    } catch (e) {
...
const messageServer = http.createServer((req, res) => {
    ...
    req.on('end', async () => {
        try {
            ...
            if (dm.length) {
                ids.push(String(dm[0].user_id_1), String(dm[0].user_id_2))
            } else {
                const [rows] = await db.execute(
                    `SELECT user_id FROM group_channel_recipients WHERE channel_id=?`,
                    [channelId]
            ...
            for (const client of wss.clients) {
                if (client.readyState === 1 && client.user && ids.includes(String(client.user.id)))
                    send(client, { op: 0, s: ++client.seq, t: 'MESSAGE_CREATE', d: message })
            }
...
wss.on('connection', (ws, req) => {
    ...
    try {
        ...
        console.error('Failed to parse connection URL:', req.url, e)
    }

    ws.zlibHeaderSent = false
    ws.sendPayload = createSender(ws, wantsZlib)
    ...
    ws.on('message', async raw => {
        ...
        catch { return ws.close(4000, 'Invalid payload') }
        ...
        try {
            ...
            if (op === 2) {
                ...
                //console.log(raw.toString()) // important debug
                ...
                for (const f of friendList) {
                    ...
                    channels.push({ id: channelId, type: 1, recipients: [f], last_message_id: null })
                }

                const groupChannels = await getGroupChannels(ws.user.id)
                channels.push(...groupChannels)

                const presences = friendList.map(f => ({
                    user: { id: f.id },
                ...
                const uniqueUsers = [
                    ...new Map([
                        ...friendList, ...blockedUsers, ...incoming, ...outgoing,
            ...
            if (op === 3 && ws.user) {
                ...
                ws.presence = { since: d?.since ?? null, status, afk: d?.afk ?? false, game: d?.game ?? null }
            ...
            if ([4, 6, 13, 14].includes(op)) return

            console.log('Unhandled op:', op, d)
        } catch (e) {
            console.error(`Error handling op ${op} for ${ws.user?.username || 'unauthenticated'}:`, e)
    ...
    ws.on('close', async (code, reason) => {
        ...
        try {
            ...
            if (statuses.includes(status) && status !== 'invisible') s.last_status = status
            s.status = 'invisible'
